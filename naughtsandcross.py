...
#!! there is a keyerror which occurs when running the code on vs code, if ran again you will be able to play the game


import sys, tty
#from stack overflow https://stackoverflow.com/questions/63144507/how-to-clear-keyboard-event-buffer-pynput-keyboard
import termios
...
def place_counter(player):
    ...
    ask = input(colour + str(player) + " type a coordinate: ")
    ...
    for y in range(3):
        for x in range(3):
            ...
            if draw_coord:
                count +=1
        print('')
#if all the places have been filled then the game is deemed and over.
    if count == 9:
        print("game over!")
        return False
    else:
...
#this while loop keeps the game running so that each player can have a turn
turn_counter = 0
while playing:
    if turn_counter % 2 == 0:
        #print("player 1")
